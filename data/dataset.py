    ...
    def __init__(self):
        ...
        with open('../data/mids_to_words.csv', 'r') as file:
            for line in file:
                ...
                mids_to_words[mid] = word
        words_to_mids = { word: mid for mid, word in mids_to_words.items() }
        mids_to_words = { mid: word for word, mid in words_to_mids.items() }
        # everything below is in english word instead of ID
        fb_all_entities = set([])
        fb_all_relations = set([])
        fb_pairs_to_relations = {}
        with open('../data/relations_of_top_500k.csv', 'r') as file:
            for line in file:
                e1, r, e2 = line[:-1].split(',')
                if e1 not in mids_to_words or e2 not in mids_to_words:
                    continue
                w1 = mids_to_words[e1]
                w2 = mids_to_words[e2]
